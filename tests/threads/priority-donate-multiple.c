...
test_priority_donate_multiple (void) 
{
  ...
  ASSERT (thread_get_priority (thread_current()) == PRI_DEFAULT);
  ...
  msg ("Main thread should have priority %d.  Actual priority: %d.",
       PRI_DEFAULT + 1, thread_get_priority (thread_current()));

  thread_create ("b", PRI_DEFAULT + 2, b_thread_func, &b);
  msg ("Main thread should have priority %d.  Actual priority: %d.",
       PRI_DEFAULT + 2, thread_get_priority (thread_current()));
  ...
  msg ("Main thread should have priority %d.  Actual priority: %d.",
       PRI_DEFAULT, thread_get_priority (thread_current()));
}
