...
thread_block (void) 
{
 ...
 // printf("Thread %s in thread_block blocking ...\n", thread_current()->name);

  thread_current ()->status = THREAD_BLOCKED;
  schedule ();

  // printf("Thread %s in thread_block unblocking ...\n", thread_current()->name);
}
...
thread_yield (void) 
{
 ...
 // printf("thread %s in thread_yield 1\n", cur->name);
  old_level = intr_disable ();
  if (cur != idle_thread) 
  ...
  schedule ();
  //printf("thread %s in thread_yield 2\n", cur->name);
  intr_set_level (old_level);
}
...
void
thread_recompute_priority(void)
{
  struct thread* crt_thread = thread_current();
  int maximum_prio = crt_thread->real_priority;
  struct list_elem *lock_elem;
  struct list_elem *thread_elem;

  for(lock_elem = list_begin(&crt_thread->acquired_locks_list); lock_elem != list_end(&crt_thread->acquired_locks_list);
      lock_elem = list_next(lock_elem))
  {
    struct lock* l = list_entry (lock_elem, struct lock, acquired_lock_list_elem);
    for(thread_elem = list_begin(&l->waiters);thread_elem != list_end(&l->waiters); thread_elem = list_next(thread_elem))
    {
...
thread_donate_priority(struct lock* lock){
  ...
  if(lock->holder->waited_lock != NULL)
  {
    thread_donate_priority(lock->holder->waited_lock);
  }
}
  ...
  for (;;) 
    {
      ...
      intr_disable ();
      //printf("Idle thread is blocking ....\n");
      thread_block ();
      //printf("Idle thread is unblocking ...\n");
...
//    list_sort(&ready_list, compare_threads, NULL);
